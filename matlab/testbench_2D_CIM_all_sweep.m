...
for iii = 0
    ...
    %% Input excel details
    ip_file_name = './inputs/tb_inputs_2D_22nm.xlsx';
    system_sheet = 'system';
    chip_sheet = 'chip';
...
%     tsv_height = [10]*1e-6;
%     tsv_map_clustered = [3.6E-03 0.1E-03 2.49E-03 9.59E-03 1.00E-01 5.00E-02]; %clustered TSV distribution
...
%     metal_thick_8 = [144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6 144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6];
    ...
    chip(1).Xsize = 0.0656+1e-4;
    chip(1).Ysize = 0.0656+1e-4;
...
%         chip(1).map = [];
    ...
    logic_map = logic_power_map_gen(x_dim_mem + x_margin_MEM + x_margin_adc, y_margin_adc, nyADC, nxADC, block_dimension_adc, y_pitch_adc, x_pitch_adc, per_ADC_power);
    A = logic_power_map_gen((x_dim_mem + x_margin_MEM + x_dim_adc/2 + pool_gb_xsize_adc + x_margin_adc), y_margin_adc, nyADC, nxADC, block_dimension_adc, y_pitch_adc, x_pitch_adc, per_ADC_power);
    logic_map = [logic_map ; A];
    P1 = [(x_dim_mem + x_margin_MEM + x_dim_adc/2) 0 (pool_gb_xsize_adc/2) y_dim_pool pool_power 0];
    GB1 = [(x_dim_mem + x_margin_MEM + x_dim_adc/2) (y_dim_pool) (pool_gb_xsize_adc/2) y_dim_gb gb_power 0];
    P2 = [(x_dim_mem + x_margin_MEM + x_dim_adc/2 + pool_gb_xsize_adc/2) (y_dim_gb) (pool_gb_xsize_adc/2) y_dim_pool pool_power 0];
    GB2 = [(x_dim_mem + x_margin_MEM + x_dim_adc/2 + pool_gb_xsize_adc/2) 0 (pool_gb_xsize_adc/2) y_dim_gb gb_power 0];
    logic_map = [logic_map ; P1 ; GB1 ; P2 ; GB2];
    chip(1).tsv_map = [0.1E-03 0.1E-03 0.0245 0.0245 1.00E-01 5.00E-02];
    die1_map = readmatrix('./3DCIM/baselinemap_22nm_ResNet150.csv'); %Bottom
    chip(1).power = 5.15E+01;
    chip(1).map = [die1_map];
    chip(1).blk_name = cell(103392+103392, 1);
    chip(1).blk_name(:) = cellstr('');
    chip(1).blk_num = [103392+103392];
    system.version = 0;
...
%                     end
                    ...
                    fprintf('Total die power: %12.3e W\n', chip(1).power);
