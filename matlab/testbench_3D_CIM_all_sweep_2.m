...
%     % L on M
    ...
    for m = [4]
        ...
        for n = [1]
            for p = [100]% 25 50 100]
                chip(1).power(1) = 100; % Die 1 (TOP) power
                chip(1).power(2) = 25;
                ...
                %% TOV definition
                chip(1).TOV.d = tsv_diameter(1);%*1E-6;
                ...
                if embedded_die_count == 1
                    ...
                    chip(1).blk_num = [0 1];
                    die2_map = findpower(system,chip, chip(1).power(2), 1);
                elseif embedded_die_count == 2
                    % 2
                    chip(1).blk_num = [0 2];
                    die2_map = findpower(system,chip, chip(1).power(2), 2);
                elseif embedded_die_count == 4
                    chip(1).blk_num = [0 4];
                    die2_map = findpower(system,chip, chip(1).power(2), 4);
                else
                    chip(1).blk_num = [0 6];
                    die2_map = findpower(system,chip, chip(1).power(2), 6);
                end
