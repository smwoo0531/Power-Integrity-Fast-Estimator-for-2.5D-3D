...
% 1. Added support for 3D stack modeling
% 2. TSV pitch for stacked dies need not be same as interposer/bridge TSV pitch, so changing that in calParasitics.m
...
%     tsv_diameter = readvars(ip_file_name, 'Sheet', block_level_specs_sheet, 'Range', 'C6:C6'); %units: m
    ...
    for i = 1:1:length(tsv_diameter)
        ...
        chip(1).TSV.d = tsv_diameter(i);
        chip(1).TSV.px = tsv_diameter(i) * 10; %TSV X pitch (p2p/g2g)
...
%         
        chip(1).c4.px = c4_pitch(i);
        chip(1).c4.py = c4_pitch(i);
        chip(1).c4.d = c4_dia(i);
        chip(1).c4.h = c4_dia(i);
