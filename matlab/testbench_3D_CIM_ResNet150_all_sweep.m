...
for iii = 0
    ...
    %% Input excel details
    ip_file_name = './inputs/tb_inputs_clust_4tiers.xlsx';
    %ip_file_name = './inputs/tb_inputs_clust_2tier_rdl.xlsx';
%     ip_file_name = './inputs/tb_inputs_unif_3tiers.xlsx';
    system_sheet = 'system';
    chip_sheet = 'chip';
    ...
    chip(1).Metal.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E10:H10');
    chip(1).Metal.p = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E11:H11'); % wire resistivity
    chip(1).Metal.ar = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E12:T12');

    chip(1).Metal.pitch = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E13:T13')*2;
    chip(1).Metal.thick = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E14:T14');

    chip(1).Via.R = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E16:T16')*450;
    chip(1).Via.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E17:T17')/45;

    chip(1).cap_per = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E19:H19');
    chip(1).cap_th = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E20:H20'); %capacitance effective thicknee (used for capacitance value calculation)

    chip(1).power = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E22:H22'); %total power dissipation of each die
    ...
    sim_case = [1];
     ...
     ubump_pitch = [160]*1e-6; % Design Parameter 1
     ubump_dia = [80]*1e-6;
    %ubump_pitch = [120]*1e-6; % Design Parameter 1
    %ubump_dia = [60]*1e-6;
...
%     tsv_pitch = [2, 6, 10, 20, 30, 40]*1e-6;
%     tsv_diameter = [1, 3, 5, 10, 15, 20]*1e-6;  % Design Parameter 2
%     tsv_height = [10, 30, 50, 100, 150, 200]*1e-6;
    tsv_pitch = [2]*1e-6;
    tsv_diameter = [1]*1e-6;  % Design Parameter 2
    tsv_height = [10]*1e-6;

    %Naive vs balanced
    tsv_map_clustered = [2.575E-03 0.1E-03 1.76E-03 6.81E-03 1.00E-01 5.00E-02]; %clustered TSV distribution
    tsv_map_uniform = [0.1E-03 0.1E-03 6.81E-03 6.81E-03 1.00E-01 5.00E-02]; %uniform TSV distribution
    
%     %2D vs 3D (22nm):
%     tsv_map_clustered = [10.605E-03 0.1E-03 1.07E-03 22.18E-03 1.00E-01 5.00E-02]; %clustered TSV distribution
%     tsv_map_uniform = [0.1E-03 0.1E-03 9.59E-03 9.59E-03 1.00E-01 5.00E-02]; %uniform TSV distribution
    
%     tsv_map_clustered = [3.8E-03 0.1E-03 0.05E-03 7.55E-03 1.00E-01 5.00E-02]; %clustered TSV distribution
%     tsv_map_uniform = [0.1E-03 0.1E-03 7.55E-03 7.55E-03 1.00E-01 5.00E-02]; %uniform TSV distribution
    ...
    metal_p = [18e-9 18e-9 18e-9]; % wire resistivity
...
%     metal_thick_8 = [144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6 144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6];
    ...
    chip(1).Xsize = 0.0245+1e-4;
    chip(1).Ysize = 0.0245+1e-4;
    ...
    chip(1).power(1) = pwr_logic;
%         chip(1).map = [];
    die1_map = logic_power_map_gen(x_margin_adc, y_margin_adc, nyADC, nxADC, block_dimension_adc, y_pitch_adc, x_pitch_adc, per_ADC_power);
    ...
    die1_map = [die1_map ; P1 ; GB1 ; P2 ; GB2];
%     chip(1).blk_name = cell((nADC*2)+4, 1);
%     chip(1).blk_name(:) = cellstr('');

    %Memory (chip 2) power map definition
    chip(1).power(2) = pwr_mem;
%         chip(2).map = [];
    die2_map = logic_power_map_gen(x_margin_MEM, y_margin_MEM, nyMEM, nxMEM, block_dimension_mem, y_pitch_mem, x_pitch_mem, per_MEM_power);
    ...
    die2_map = [die2_map ; B]; % this combines into single array
    %chip(1).power(2) = 0;
    %die2_map = [ ; ];
%     chip(2).blk_name = cell(nMEM*2, 1);
%     chip(2).blk_name(:) = cellstr('');
    
    %Logic (chip 3) power map definition
    chip(1).power(3) = pwr_logic;
%         chip(1).map = [];
    die3_map = logic_power_map_gen(x_margin_adc, y_margin_adc, nyADC, nxADC, block_dimension_adc, y_pitch_adc, x_pitch_adc, per_ADC_power);
    A = logic_power_map_gen((x_dim_adc/2 + pool_gb_xsize_adc + tsv_xsize_adc + x_margin_adc), y_margin_adc, nyADC, nxADC, block_dimension_adc, y_pitch_adc, x_pitch_adc, per_ADC_power);
    die3_map = [die3_map ; A];
    P1 = [(x_dim_adc/2) 0 (pool_gb_xsize_adc/2) y_dim_pool pool_power 0];
    GB1 = [(x_dim_adc/2) (y_dim_pool) (pool_gb_xsize_adc/2) y_dim_gb gb_power 0];
    P2 = [(x_dim_adc/2 + pool_gb_xsize_adc/2 + tsv_xsize_adc) (y_dim_gb) (pool_gb_xsize_adc/2) y_dim_pool pool_power 0];
    GB2 = [(x_dim_adc/2 + pool_gb_xsize_adc/2 + tsv_xsize_adc) 0 (pool_gb_xsize_adc/2) y_dim_gb gb_power 0];
    die3_map = [die3_map ; P1 ; GB1 ; P2 ; GB2];
%     chip(1).blk_name = cell((nADC*2)+4, 1);
%     chip(1).blk_name(:) = cellstr('');

    chip(1).tsv_map = [0.010163610935503574 0.1E-03 0.014497092243268674 0.0245 1.00E-01 5.00E-02]; %clustered TSV distribution
    %chip(1).tsv_map = [0.1E-03 0.1E-03 0.0245 0.0245 1.00E-01 5.00E-02];
    %chip(1).tsv_map = [0.001190609111 0.001190609111 0.00155939111 0.00155939111 1.00E-01 5.00E-02];
    %tsv_map_uniform = [0.1E-03 0.1E-03 6.81E-03 6.81E-03 1.00E-01 5.00E-02]; %uniform TSV distributio
    %chip(1).map = [die1_map ; die2_map; die3_map];
    %chip(1).blk_num = [0 0];
    chip(1).power(4) = 34.4040395;
    chip(1).power(3) = 6.10;
    chip(1).power(2) = 2.2;
    chip(1).power(1) = 3.75E-1; % TOP DIE
    die1_map = readmatrix('./3DCIM/memPowermap_4tier_top_unbalanced.csv'); %TOP DIE
    die2_map = readmatrix('./3DCIM/memPowermap_4tier_bottom_unbalanced.csv'); %Bottom
    die3_map = readmatrix('./3DCIM/logPowermap_4tier_top_unbalanced.csv'); %Bottom
    die4_map = readmatrix('./3DCIM/logPowermap_4tier_bottom_unbalanced.csv'); %Bottom
    chip(1).map = [die1_map; die2_map; die3_map; die4_map];
    chip(1).blk_name = cell(51696+51696+51696+51696, 1);
    chip(1).blk_name(:) = cellstr('');
    chip(1).blk_num = [51696 51696 51696 51696];

    % chip(1).map = [[]; []; []; []];
    % chip(1).blk_name = cell(1, 1);
    % chip(1).blk_name(:) = cellstr('');
    % chip(1).blk_num = [0 0 0 0];
    
        chip(1).TSV.Nbundle = 1;
...
%                     chip(1).Metal.N = [metal_layers(m) metal_layers(m) metal_layers(m)];
