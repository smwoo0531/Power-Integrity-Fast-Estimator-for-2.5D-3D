...
%     ubump_pitch = [40, 55, 80, 120, 160, 200]*1e-6; % Design Parameter 1
%     ubump_dia = [20, 25, 40, 60, 80, 100]*1e-6;
...
%     c4_dia = [20, 25, 40, 50, 50]*1e-6;
    ...
    chip(1).Ysize = 0.01;
    % testing for 3-tier
    %chip(1).Xsize = 0.0173+1e-4;
    %chip(1).Ysize = 0.0173+1e-4;
    ...
    chip(1).TSV.thick = 40E-6;

    tsv_map_clustered_1um_RESNET_22nm = [0.005 0 0.005 0.0023125 0.016 5.00E-02]; %clustered TSV distribution
...
%     % L on M
    ...
    chip(1).power(1) = 10; % Die 1 (TOP) power
    chip(1).power(2) = .01; %Die 2 (BOTTOM) power
    %chip(1).power(2) = 2.77947487; %Die 2 (BOTTOM) power

    % Testing imported memory map
    %die1_map = readmatrix('./3DCIM/memPowermap_2tier_unbalanced.csv'); %TOP DIE
    %die2_map = readmatrix('./3DCIM/logicPowermap_2tier.csv'); %Bottom
    die1_map = [];
    die2_map = [];
    chip(1).blk_num = [0 0];
    chip(1).map = [die1_map; die2_map];
    % TSV map: LL_X LL_Y x_length y_length
    chip(1).tsv_map = [0.0025 0.0025 0.0075 0.0075 1.00E-01 5.00E-02]; %clustered TSV distribution
    %chip(1).tsv_map = [0 0 chip(1).Xsize chip(1).Ysize 5.00E-02];
    %chip(1).tsv_map = [0.001190609111 0.001190609111 0.00155939111 0.00155939111 1.00E-01 5.00E-02];
    %chip(1).tsv_map = [0.0074 0.1E-03 0.002326 0.0173 5.00E-02]; %clustered TSV distribution
    %chip(1).tsv_map = [0.1E-03 0.1E-03 chip(1).Xsize-0.1E-03 chip(1).Ysize-0.1E-03 5.00E-02]; %clustered TSV distribution
    ...
    %chip(1).TSV.d = 1E-6;
    %chip(1).tsv_map = [4.8997e-3 1e-4 1.96e-4 9.8e-3 1.00E-01 5.00E-02];
    %chip(1).tsv_map = [0 0 chip(1).Xsize chip(1).Ysize 1.00E-01 5.00E-02];
