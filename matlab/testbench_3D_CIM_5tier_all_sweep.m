...
for iii = 0
    ...
    %% Input excel details
    ip_file_name = './inputs/tb_inputs_clust_5tiers.xlsx';
...
%     ip_file_name = './inputs/tb_inputs_unif_3tiers.xlsx';
    ...
    chip(1).Metal.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E10:I10');
    chip(1).Metal.p = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E11:I11'); % wire resistivity
    chip(1).Metal.ar = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E12:X12');

    chip(1).Metal.pitch = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E13:X13')*2;
    chip(1).Metal.thick = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E14:X14');

    chip(1).Via.R = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E16:X16')*450;
    chip(1).Via.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E17:X17')/45;

    chip(1).cap_per = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E19:I19');
    chip(1).cap_th = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E20:I20'); %capacitance effective thicknee (used for capacitance value calculation)

    chip(1).power = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E22:I22'); %total power dissipation of each die
    ...
    sim_case = [1];
     ...
     %ubump_pitch = [55, 80, 160, 200]*1e-6; % Design Parameter 1
     %ubump_dia = [25 40 80 100]*1e-6;
...
%     metal_thick_8 = [144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6 144e-9 144e-9 144e-9 144e-9 144e-9 144e-9 324e-9 7e-6];
    ...
    chip(1).Xsize = 0.0201+1e-4;
    chip(1).Ysize = 0.0201+1e-4;
...
%     chip(1).blk_name(:) = cellstr('');
    ...
    chip(1).tsv_map = [0.009094314974832757 0.1E-03 0.011086250582728432 0.0201 5.00E-02]; %clustered TSV distribution
    %chip(1).map = [die1_map ; die2_map; die3_map];
    %chip(1).blk_num = [0 0];
    chip(1).power(5) = 17.29481546;
    chip(1).power(4) = 1.46;
    chip(1).power(3) = 2.15E-01;
    chip(1).power(2) = 0.1;
    chip(1).power(1) = 4.40E-02; % TOP DIE
    die1_map = readmatrix('./3DCIM/memPowermap_5tier_1_balanced.csv'); %TOP DIE
    die2_map = readmatrix('./3DCIM/memPowermap_5tier_2_balanced.csv'); %Bottom
    die3_map = readmatrix('./3DCIM/memPowermap_5tier_3_balanced.csv'); %Bottom
    die4_map = readmatrix('./3DCIM/memPowermap_5tier_4_balanced.csv'); %Bottom
    die5_map = readmatrix('./3DCIM/logicPowermap_5tier.csv'); %Bottom
    chip(1).map = [die1_map; die2_map; die3_map; die4_map; die5_map];
    chip(1).blk_name = cell(38880+19440+19440, 1);
    chip(1).blk_name(:) = cellstr('');
    chip(1).blk_num = [9720 9720 9720 9720 38880];
