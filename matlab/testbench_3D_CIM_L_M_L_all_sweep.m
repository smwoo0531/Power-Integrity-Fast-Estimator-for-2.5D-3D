...
for iii = 0
    ...
    %% Input excel details
    ip_file_name = './inputs/tb_inputs_clust_3tiers.xlsx';
...
%     ip_file_name = './inputs/tb_inputs_unif_3tiers.xlsx';
    ...
    chip(1).Metal.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E10:G10');
    chip(1).Metal.p = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E11:G11'); % wire resistivity
    chip(1).Metal.ar = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E12:P12');

    chip(1).Metal.pitch = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E13:P13')*2;
    chip(1).Metal.thick = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E14:P14');

    chip(1).Via.R = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E16:P16')*450;
    chip(1).Via.N = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E17:P17')/45;

    chip(1).cap_per = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E19:G19');
    chip(1).cap_th = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E20:G20'); %capacitance effective thicknee (used for capacitance value calculation)

    chip(1).power = readmatrix(ip_file_name, 'UseExcel', true, 'Sheet', chip_sheet, 'Range', 'E22:G22'); %total power dissipation of each die
...
%     chip(1).blk_name(:) = cellstr('');
    ...
    chip(1).tsv_map = [0.010023838561399358 0.1E-03 0.014357319869164458 0.0245 5.00E-02]; %clustered TSV distribution
    %chip(1).map = [die1_map ; die2_map; die3_map];
    %chip(1).blk_num = [0 0];
    chip(1).power(3) = 17.29481546;
    chip(1).power(2) = 1.6913956;
    chip(1).power(1) = 0.14298942; % TOP DIE
    die1_map = readmatrix('./3DCIM/memPowermap_3tier_top_unbalanced.csv'); %TOP DIE
    die2_map = readmatrix('./3DCIM/memPowermap_3tier_bottom_unbalanced.csv'); %Bottom
    die3_map = readmatrix('./3DCIM/logicPowermap_3tier.csv'); %Bottom
    chip(1).map = [die1_map; die2_map; die3_map];
    chip(1).blk_name = cell(38880+19440+19440, 1);
    chip(1).blk_name(:) = cellstr('');
    chip(1).blk_num = [19440 19440 38880];
