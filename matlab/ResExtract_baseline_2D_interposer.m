                    ...
                    for j = 1:chip(ii).Ny
                        ...
                        for i = 1:chip(ii).Nx
                            ...
                            if LineOrient == 0                           
                                ...
                                %if chip(ii).mesh_grid.custom == 1                              
                                %    fprintf('area %d , gridx_temp: %d, , gridy: %d,via_R: %d \n', area, gridx_temp , gridy, via_R);        
                                %else 
                                %    fprintf('area %d , gridx: %d, , gridy: %d,via_R: %d \n', area, gridx , gridy, via_R); 
                                %end
                                if i < chip(ii).Nx
                                    row(pointer) = id;
                                    col(pointer) = frontId;
                                ...
                                if IOmapTSV(j,i) == 1
                                    if k == chip(ii).N
                                        ...
                                        % bottom die && lateral metal (diff 2const) 
                                        bottomId = id + 2*const;
                                ...
                                if IOmap(j,i) == 1
                                     ...
                                     if chip(ii).Xmesh(i)>= chip(ii).tsv_map(1) && ...
                                         ...
                                         if k ~= chip(ii).N % not bottom die (N=1)
                            ...
                            if LineOrient == 0                           
                                ...
                                if IOmapc4(j,i) == 1
                                    if k == chip(ii).N
                                        [indX, indY] = chip2pkgId(chip(ii).Xmesh(i), chip(ii).Ymesh(j), chip(ii), system.pkg);
                                        bottomId = system.chip.numV + indX + (indY - 1)*system.pkg.Nx;
                                        via_R = chip(ii).TSV.R+chip(ii).c4.R_map(j,i);
                                        %c4count=c4count+1;
                                        %c4count
                                        if system.debug_id == 1                            
                                             fprintf('c4 tsv, id: %d , bottomId: %d\n',  id, bottomId);
                                        end                                       
                                        row(pointer) = id;
                                        col(pointer) = bottomId;
                                        val(pointer) = via_R;
