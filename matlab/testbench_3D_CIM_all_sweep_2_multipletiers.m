...
%     % L on M
    ...
    chip(1).power(1) = 25; % Die 1 (TOP) power
    chip(1).power(2) = 1; %Die 2 (BOTTOM) power
    embedded_die_count_bottom = 1; % # of dies embedded in bottom tier
  
    % Vary the TSV diameter
    %chip(1).TSV.d = 1E-6;
    chip(1).tsv_map = [0 0 chip(1).Xsize chip(1).Ysize 1.00E-01 5.00E-02];
    chip(1).tov_map = [0 0 chip(1).Xsize chip(1).Ysize 1.00E-01 5.00E-02];

    for m = [6]
        fprintf('-------------EMBEDDED DIES = %d----------------\n', m);
        fprintf('------------------------------------------------\n');
        embedded_die_count_bottom = m;
        embedded_die_count_top = m;

        for n = [100]
            for p = [25]% 25 50 100]
                chip(1).power(1) = 25; % Die 1 (TOP) power
                chip(1).power(2) = 0; % RDL layer
                chip(1).power(3) = 100; % Bottom die
                ...
                %% TOV definition
                chip(1).TOV.d = tsv_diameter(1);%5*1E-6;
                chip(1).TOV.px = chip(1).TOV.d*2;
                chip(1).TOV.thick = tsv_height(1);
                chip(1).TOV.Nbundle = 1;
                chip(1).TOV.rou = chip(1).TSV.rou;
                chip(1).TOV.liner = chip(1).TSV.liner;
                chip(1).TOV.mu = chip(1).TSV.mu;
                chip(1).TOV.contact = chip(1).TSV.contact;

                % Creating giant for loop to test all cases
                if embedded_die_count_bottom == 1
                    % single
                    %die2_map = [0.0068 0.0068 0.0136 0.0136 1 0];
                    chip(1).blk_num = [0 1];
                    die2_map = findpower(system,chip, chip(1).power(3), 1);
                elseif embedded_die_count_bottom == 2
                    % 2
                    chip(1).blk_num = [0 2];
                    die2_map = findpower(system,chip, chip(1).power(3), 2);
                elseif embedded_die_count_bottom == 4
                    chip(1).blk_num = [0 4];
                    die2_map = findpower(system,chip, chip(1).power(3), 4);
                else
                    chip(1).blk_num = [0 6];
                    die2_map = findpower(system,chip, chip(1).power(3), 6);
                end

                % Creating giant for loop to test all cases
                if embedded_die_count_top == 1
                    % single
                    %die2_map = [0.0068 0.0068 0.0136 0.0136 1 0];
                    chip(1).blk_num = [0 1];
                    die1_map = findpower(system,chip, chip(1).power(1), 1);
                elseif embedded_die_count_top == 2
                    % 2
                    chip(1).blk_num = [0 2];
                    die1_map = findpower(system,chip, chip(1).power(1), 2);
                elseif embedded_die_count_top == 4
                    chip(1).blk_num = [0 4];
                    die1_map = findpower(system,chip, chip(1).power(1), 4);
                else
                    chip(1).blk_num = [0 6];
                    die1_map = findpower(system,chip, chip(1).power(1), 6);
                end

                %die1_map = [0.0045 0.0045 0.001 0.001 5 0];
                %die1_map = [0.0005 0.0085 0.001 0.001 5 0];
                %chip(1).blk_num = [1 embedded_die_count_bottom];
                %die1_map = [];
                rdl_map = [];
                chip(1).map = [die1_map; rdl_map; die2_map];
                chip(1).blk_num = [embedded_die_count_top 0 embedded_die_count_bottom];
